...
form.addEventListener("submit", (e) => {
  ...
  validateFiled(password, /^(?=.*\d)(?=.*[A-Z])(?=.*\W).{8,}$/, "Password must have 8+ chars, a number, a special char, and an uppercase.");
  if (confirmpassword.value.trim() !== password.value.trim()) {
    confirmpassword.nextElementSibling.nextElementSibling.textContent = "Password must match";
...
togglebutton("toggleconfirmpassword", "confirmpassword");
